...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
예시:
  python main.py                  # 기본값(100명)으로 테스트 실행
  python main.py 200              # 200명의 동시 사용자로 테스트 실행
  python main.py -h, --help       # 도움말 표시

출력 파일:
  log/load_test_results_{사용자수}users_{시간}.txt  # 요약 결과
  log/load_test_detailed_{사용자수}users_{시간}.json # 상세 로그
        """
    )
...
async def main_async(concurrent_users):
    """비동기 메인 함수"""
    # 테스트 실행
    await test_runner.run_load_test(concurrent_users)
...
def main():
    ...
    print("=" * 50)
    print(f"게임 서버 부하 테스트 시작 (동시 사용자: {concurrent_users}명)")
    ...
    # 비동기 테스트 실행
    asyncio.run(main_async(concurrent_users))
    ...
    stats.save_results_to_file(concurrent_users)
